...
// 텍스트 대화와 관련된 코드
// ############################################################################
window.springai = window.springai || {}; // springai 가 없으면 빈 객체 생성
...
// LLM 으로부터 Response 가 Chunk 단위로 오기 때문에, 스트림이용해 바이트 단위로 읽어들이도록 작성.
springai.printAnswerStreamText = async function (responseBody, targetId, chatPanelId) {
  const targetElement = document.getElementById(targetId);
  const reader = responseBody.getReader();
  const decoder = new TextDecoder("utf-8");
	let content = "";
  while (true) { // 계속해서 바이트 단위로 데이터가 넘어오기 때문에 무한루프 이용해 읽어들인다.
    const { value, done } = await reader.read();
    if (done) break; // done 이 true 면 반복문 종료
    let chunk = decoder.decode(value);
		content += chunk;
		if(!springai.isOpenTagIncomplete(chunk)) { 
      // innerHTML 은 완전하지 않은 HTML 문장을 넣으면 무시해버린다. 
      // 그래서 완전하게 만든다음에 innerHTML 수행.
