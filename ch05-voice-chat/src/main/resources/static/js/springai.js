...
springai.voice.initMic = async function (handleVoice) {
  ...
  // 값이 작을 수록 저주파에서 고주파로 튀는 부분을 잘 잡아내지 못하는 문제 발생
  // 값을 크게 줄 수록 속도가 느려지고 비용이 증가
...
springai.voice.initMediaRecorder = function (handleVoice) {
  ...
  mediaRecorder.ondataavailable = async (event) => {
	// 계속해서 음성 녹음되다 중지되는 지점에서 녹음된 데이터가 사용 가능해졌다고 판단하고 이벤트 실행
    //음성 확인이 되었고, 녹화 데이터가 있고, 현재 대화중이 아닐 경우
    if (springai.voice.voice === true && event.data.size > 0 && springai.voice.chatting === false ) {
      console.log("대화 시작");
      springai.voice.chatting = true; // false 면 대화 시작하지 않은 상태

      //MP3로 변환
      const webmBlob = event.data;
      const mp3Blob = await springai.voice.convertWebMToMP3(webmBlob); // MP3 파일로 변환
      //콜백(사용자 로직) 실행 -------------
      handleVoice(mp3Blob);
      //---------------------------------
    }
  ...
  try {
    ...
    mediaSource.addEventListener('sourceopen', async () => {
      ...
      while (true) {
        ...
        // Chunk 를 읽어들인 다음 순서대로 버퍼에 Chunk 를 추가. Promise 함수 이용해 대기한 후 순차적 처리ㅣ
      }
    });
    // 재생 시작
    audioPlayer.play();
